...
public sealed class GitSubmissionAncestryValidatorTests
{
    [Fact]
    public void ValidateAncestry_RunsMergeBaseIsAncestorFromBaseToFetchedLocalRef()
    {
        ...
        var validator = new GitSubmissionAncestryValidator(runner);

        var result = validator.ValidateAncestry(
            submission,
        ...
        Assert.Contains("submission head descends from recorded base commit", result.Decisions);
    }

    [Fact]
    public void ValidateAncestry_RejectsWhenBaseIsNotAncestorOfFetchedHead()
    {
        var runner = new RecordingGitCommandRunner(new GitCommandResult(1, string.Empty, string.Empty));
        var validator = new GitSubmissionAncestryValidator(runner);

        var result = validator.ValidateAncestry(ApprovedSubmission(), "/workspace", "refs/den-publish/submissions/sub_1424_001");

        Assert.False(result.IsPublishable);
        var failure = Assert.Single(result.Failures);
        Assert.Equal(PublishFailureCode.NonFastForward, failure.Code);
        Assert.Contains("does not descend", failure.Message, StringComparison.Ordinal);
    }

    [Fact]
    public void ValidateAncestry_FailsWhenGitCannotCheckAncestry()
    {
        var runner = new RecordingGitCommandRunner(new GitCommandResult(128, string.Empty, "fatal: not a git repository"));
        var validator = new GitSubmissionAncestryValidator(runner);
        ...
        Assert.Contains("not a git repository", failure.Message, StringComparison.Ordinal);
    }

    [Fact]
    public void ValidateAncestry_RejectsUnsafeLocalRefBeforeRunningGit()
    {
        var runner = new RecordingGitCommandRunner(new GitCommandResult(0, string.Empty, string.Empty));
        var validator = new GitSubmissionAncestryValidator(runner);

        var result = validator.ValidateAncestry(ApprovedSubmission(), "/workspace", "refs/heads/main;touch /tmp/nope");
